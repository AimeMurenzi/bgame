...
public class Util {
    ...
    public static void throwBadRequest(String message) {
        ...
        throw new WebApplicationException(
                Response.status(Status.BAD_REQUEST).type(MediaType.TEXT_PLAIN_TYPE).entity(message).build());
    }

    public static void throwUnAuthorized(String message) {
        throw new WebApplicationException(
                Response.status(Status.UNAUTHORIZED).type(MediaType.TEXT_PLAIN_TYPE).entity(message).build());
    }
}